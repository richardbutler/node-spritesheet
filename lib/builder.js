...
SpriteSheetBuilder = (function() {
  ...
  function SpriteSheetBuilder(files, options) {
    var config, key;
    this.files = files;
    ...
    this.outputConfigurations = options.output;
    delete options.output;
    if (this.outputConfigurations && Object.keys(this.outputConfigurations).length > 0) {
      for (key in this.outputConfigurations) {
        config = this.outputConfigurations[key];
        this.outputConfigurations[key] = _.extend({
          name: key
        }, options, config);
      }
    } else {
  ...
  SpriteSheetBuilder.prototype.build = function(done) {
    ...
    for (key in this.outputConfigurations) {
      ...
      ssc = new SpriteSheetConfiguration(this.files, config);
      if (!baseConfig || config.pixelRatio > baseConfig.pixelRatio) {
        baseConfig = config;
      }
      this.configs.push(ssc);
    }
...
SpriteSheetConfiguration = (function() {
  ...
  SpriteSheetConfiguration.prototype.build = function(callback) {
    ...
    console.log("--------------------------------------------------------------");
    this.derived = SpriteSheetConfiguration.baseConfiguration !== this;
