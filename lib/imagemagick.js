...
ImageMagick = (function() {
  ...
  ImageMagick.prototype.composite = function(options, callback) {
    var command, filepath, height, images, interpolation, width,
      _this = this;
    filepath = options.filepath, images = options.images, width = options.width, height = options.height, interpolation = options.interpolation;
    ...
    return this.exec(command, function(error, stdout, stderr) {
      ...
      compose = function(image, next) {
        ...
        return _this.composeImage(filepath, image, interpolation, next);
      };
  ...
  ImageMagick.prototype.composeImage = function(filepath, image, downsampling, callback) {
    var command;
    downsampling || (downsampling = "Lanczos");
    command = "      composite      -geometry " + image.width + "x" + image.height + "+" + image.cssx + "+" + image.cssy + "      -filter " + downsampling + "      " + image.path + " " + filepath + " " + filepath + ".tmp            &&      mv " + filepath + ".tmp " + filepath + "    ";
